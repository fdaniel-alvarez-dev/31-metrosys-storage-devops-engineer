...
def demo_mode() -> None:
    ...
    report_path = ARTIFACTS_DIR / "gitops_guardrails.json"
    guard = run([sys.executable, "tools/gitops_guardrails.py", "--format", "json", "--out", str(report_path)])
    if guard.returncode != 0:
        fail("GitOps guardrails failed (demo mode must be offline).", output=guard.stdout)

    report = load_json(report_path)
    if report.get("summary", {}).get("errors", 0) != 0:
        fail("GitOps guardrails reported errors.", output=json.dumps(report.get("findings", []), indent=2))
